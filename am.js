javascript: function AtaqueAS() {
    ...
    function wait(time_miliseconds) {
        return new Promise((resolve, reject) => setTimeout(resolve, time_miliseconds));
    }
    $(document).ready(async function() {
        if (window.game_data.screen == 'map') {
            ...
            let Enviar = (template, i) => {
                $.ajax({
                    url: "/game.php?village=" + game_data.village.id + "&screen=am_farm&mode=farm&ajaxaction=farm&template_id=" + TWMap.urls.ctx.mp_farm_a + "&target=" + ids[i] + "&source=" + game_data.village.id + "&json=1&&h=" + csrf_token + "&client_time=" + Math.round(Timing.getCurrentServerTime() / 1e3),
                    data: {},
                    type: "GET",
                    dataType: "json",
                    headers: {
            ...
            let AtualizarTropas = async () => {
                ...
                $.ajax({
                    ...
                    success: async (data) => {
                        ...
                        let template = $(data).find('form').eq(0).attr('action').split("template_id=")[0].split("&")[0];
                        if (spy > 0 && template !== undefined) {
                            for (let i = 0;
                                (i < spy) && (i < ids.length); i++) {
                                Enviar(template, i);
                                await wait(aleatorio(250, 350));
                            }
                            UI.SuccessMessage('Ja acabou filho da puta do carai!', 5000);
                        }
                    }
                });
            };
            await AtualizarTropas();
        } else {
